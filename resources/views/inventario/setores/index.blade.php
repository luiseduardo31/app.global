                ...
                <nav class="flex-sm-00-auto ml-sm-3" aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-alt">
                        ...
                        <li class="breadcrumb-item" aria-current="page">
                            <a class="link-fx" href="">Setores Cadastrados</a>
                        </li>
                    </ol>
                </nav>
            </div>
                        ...
                        <div class="block-content block-content-full">
                            ...
                            <table id="contact-detail" class="responsive display nowrap table table-bordered table-striped table-vcenter" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="text-center">{{$setor->SetorID}}</td>
                                        <td>{{$setor->setor}}</td>
                                        <td class="text-center">{{$setor->grupo}}</td>
                                        <td>{{$setor->obsSetor}}</td>
                                        <td> 
                                            <div class="btn-group">
                                                <button type="button" class="btn btn-sm btn-light js-tooltip-enabled" data-toggle="tooltip" title="Editar Registro" data-original-title="Editar">
                                                    <a href="{{route('setores.edit', $setor->SetorID)}}">
                                                        <i class="fa fa-fw fa-pencil-alt"></i>
                                                    </a>
                                                </button>
                                                <form action="{{route('setores.destroy',$setor->SetorID)}}" method="POST" enctype="multipart/form-data">
                                                    @csrf
                                                    @method('DELETE')
                                                ...
                                                onclick="return confirm('Deseja realmente excluir o setor {{$setor->setor}}?');">
